...
::: {.acerca-resumen}
**Servicio de predicción estacional invernal (NDJFM)** de aportaciones a embalses peninsulares, desarrollado por **Tragsatec** en el HPC AEMET–ECMWF como parte de la colaboración AEMET–Tragsatec 2024–2026.

El sistema construye, para cada embalse, un modelo estadístico **PLS** que relaciona la precipitación invernal en rejilla con la aportación observada, lo entrena con observaciones históricas y lo aplica al hindcast del modelo dinámico **ECMWF SEAS5.1** previamente **calibrado mediante Quantile Delta Mapping (QDM)**.

El resultado es una predicción probabilística por terciles (seco / normal / húmedo) acompañada de métricas de pericia objetivas para cada uno de los **183 embalses** con señal predictiva suficiente.
:::

## 1 · Objetivo y contexto del servicio
...
En España la señal predecible a escala estacional en invierno está dominada por la **Oscilación del Atlántico Norte (NAO)** y otros patrones de variabilidad climática de gran escala. Tradicionalmente esta señal ha sido difícil de capturar por los modelos dinámicos, pero en los últimos años (i) los modelos de circulación general han mejorado su pericia en el Atlántico Norte y (ii) se han consolidado técnicas de post-proceso y calibración que permiten extraer información útil de los hindcasts.

Este servicio se inscribe en ese marco. Su objetivo es **proporcionar, para cada embalse español, una predicción probabilística de la aportación invernal acumulada junto con una estimación rigurosa de su pericia**, replicando y extendiendo la metodología establecida por Jesús Gutiérrez (AEMET) y adaptándola al sistema SEAS5.1 sobre rejilla ROCIO_IBEB de 5 km.

::: {.acerca-callout .info}
[▸ Variable predicha]{.callout-title}
Aportación total invernal $Q$ (hm³) definida como la suma de las aportaciones mensuales `entmes` de noviembre a marzo (NDJFM) para cada año hidrológico.
:::
...
- **Base de datos de embalses peninsulares** con series mensuales de aportación `entmes` en hm³.
...
- **Variable**: precipitación total (`tp`) acumulada NDJFM.
- **Inicialización**: 1 de noviembre.
...
El servicio se ejecuta en el HPC AEMET–ECMWF mediante un pipeline consolidado (`script6_consolidado_BSS_ROC.py`) que integra los seis pasos en una sola pasada vectorizada:
...
<svg viewBox="0 0 900 480" xmlns="http://www.w3.org/2000/svg" role="img" aria-label="Diagrama de flujo del pipeline">
  ...
  <text class="step-num" x="30" y="58">ENTRADA 1</text>
  ...
  <text class="step-num" x="30" y="158">ENTRADA 2</text>
  <text class="lbl"     x="30" y="178">SEAS5.1 hindcast</text>
  <text class="lbl-sub" x="30" y="195">25 miembros · 1993–2016</text>

  <rect class="box-input" x="20"  y="240" width="200" height="70" rx="6"/>
  <text class="step-num" x="30" y="258">ENTRADA 3</text>
...
Las predicciones son **probabilísticas**, no deterministas. Una predicción de "70% de probabilidad de tercil húmedo" no significa que el invierno vaya a ser húmedo: significa que, en el largo plazo, de cada 10 inviernos con ese pronóstico, en torno a 7 caerán en el tercil húmedo. La decisión de gestión debe ponderar esta probabilidad con el coste/beneficio de cada escenario.

### 7.2 · Pericia heterogénea entre demarcaciones

La pericia no es uniforme. El visor de **Análisis por cuenca** muestra que las demarcaciones del cuadrante NW (Miño-Sil, Duero, Tajo) presentan correlaciones medianas QDM en torno a $r \approx 0.27$–$0.35$, mientras que Guadalquivir y Segura quedan por debajo de $r \approx 0.10$. Esto es consistente con la influencia diferencial de la NAO y la mayor predecibilidad de la fachada atlántica.
...
En paralelo a este servicio de predicción estacional, la misma familia metodológica basada en PLS se está aplicando a **proyecciones climáticas** para estimar el impacto del cambio climático sobre las aportaciones futuras a embalses (Gutiérrez-Fernández, Sanfiz & Rodríguez-Guisado, AEMET–Tragsatec).
